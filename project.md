...
- SFTP 本地、远程和传输列表采用主体可伸缩、16px 滚动条独立列的布局，并让滚动条使用自身列的布局边界，避免挤压或覆盖文件内容；底部传输记录面板按可用高度约三分之一动态布局。
- SFTP 会话按保存的会话配置持久化并恢复最近打开的本地和远程目录；传输记录仅保留在内存中，不做持久化。
- SFTP 本地文件和目录右键菜单按监听状态显示「监听目录」或「停止监听」，监听按文件/目录类型分别处理，2 秒去抖后自动上传；监听行使用跟随主题明暗偏移的背景色标记，监听状态仅在当前会话生效，关闭会话自动停止。
- MCP 已暴露 SFTP 本地监听、停止监听和查询监听列表接口，接口参数包含 `workspace_id`、`ip` 和 `title`，并校验会话身份。
- MCP 已暴露 SFTP 本地目录和远程目录切换接口，接口参数包含 `workspace_id`、`ip`、`title` 和 `path`；远程目录切换会复用会话状态持久化流程，本地目录切换要求目标会话当前已选中。
- MCP 打开会话（包括 SFTP）和切换会话接口已补充 `ip/title` 校验；会话列表和打开结果也会返回对应字段。
- 顶部会话标签列表采用主体可伸缩、固定底部滚动条的布局，避免横向滚动条被标签内容挤压。
...
- MCP `list_profiles` 会暴露会话配置的 `id`、`title` 和 `host`，其中 `title` 来自会话配置标题。
